...
export function ObjectivesForm({ projectId, objectives, competencyGaps }: Props) {
  ...
  // Columns now use textarea for wider cells
  const wideColumns: ColumnDef[] = [
    { key: "moduleName", label: "模块名称", type: "textarea", placeholder: "如：战略解码营", minWidth: "140px" },
    { key: "objective", label: "学习目标（SMART表述）", type: "textarea", width: "280px", placeholder: "学员能够..." },
    { key: "bloomLevel", label: "布鲁姆层级", type: "select", width: "140px", options: BLOOM_LEVELS },
    { key: "competency", label: "能力维度", type: "select", width: "140px", options: CAKE_DIMENSIONS },
    { key: "businessGoal", label: "对齐业务目标", type: "textarea", placeholder: "业务目标...", minWidth: "140px" },
    { key: "assessMethod", label: "评估方法", placeholder: "笔试/实操/汇报" },
    { key: "assessTiming", label: "评估时机", placeholder: "培训后1周" },
    { key: "difficulty", label: "难度", type: "number", width: "70px" },
  ];
      ...
      <Card>
        ...
        <CardContent>
          <EditableTable
            columns={wideColumns}
            data={data}
            onSave={async (rows) => { await saveObjectives(projectId, rows); setData(rows); }}
            addLabel="添加学习目标"
          />
        </CardContent>
      </Card>
