  ...
  if (items.length > 0) {
    await prisma.learnerPersona.createMany({
      data: items.map((item) => ({
        ...
        mgmtYears: item.mgmtYears || "",
        traits: item.traits || "",
        motivation: item.motivation || "",
        challenges: item.challenges || "",
        learnStyle: item.learnStyle || "",
        techLevel: item.techLevel || "",
        assessOnline: item.assessOnline || "",
        assessOffline: item.assessOffline || "",
        assessDisc: item.assessDisc || "",
        assessMbti: item.assessMbti || "",
        assess360: item.assess360 || "",
        assessCustom: item.assessCustom || "",
        trainingHistory: item.trainingHistory || "",
        projectExp: item.projectExp || "",
        trackNotes: item.trackNotes || "",
        projectId,
      })),
    });
  }
